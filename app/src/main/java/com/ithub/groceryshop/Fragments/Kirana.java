 ...
 */
public class Kirana extends Fragment {

    RecyclerView rv_kirana;
    TextView textView_noData_kirana;
    List<Product_model> productlist_kirana;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_kirana, container, false);
        rv_kirana = view.findViewById(R.id.rv_kirana);
        textView_noData_kirana = view.findViewById(R.id.tv_nodata_kirana);
        progressDialog = new ProgressDialog(getActivity());

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
        rv_kirana.setLayoutManager(linearLayoutManager);

        dbref_products = FirebaseDatabase.getInstance().getReference("Products").child("Kirana");
        productlist_kirana = new ArrayList<>();
    ...
    public void mainMethod() {
        ...
        dbref_products.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                productlist_kirana.clear();

                for (DataSnapshot getorder : dataSnapshot.getChildren()) {
                    Product_model pm = getorder.getValue(Product_model.class);
                    productlist_kirana.add(pm);
                    progressDialog.dismiss();
                }

                // when list is empty text will appear
                if (!(productlist_kirana.size() > 0)) {
                    textView_noData_kirana.setVisibility(View.VISIBLE);
                    progressDialog.dismiss();
                } else {
                    textView_noData_kirana.setVisibility(View.INVISIBLE);
                    progressDialog.dismiss();
                }

                productAdapter = new ProductAdapter(productlist_kirana, getActivity());
                Collections.sort(productlist_kirana);
                rv_kirana.setAdapter(productAdapter);
