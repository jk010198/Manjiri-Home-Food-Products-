...
public class Dryfruits extends Fragment {
    ...
    RecyclerView rv_dryfruits;
    TextView textView_noData_dryfruits;
    List<Product_model> productlist_dryfruits;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rv_dryfruits = view.findViewById(R.id.rv_dryfruits);
        textView_noData_dryfruits = view.findViewById(R.id.tv_nodata_dryfruits);
        progressDialog = new ProgressDialog(getActivity());

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
        rv_dryfruits.setLayoutManager(linearLayoutManager);

        dbref_products = FirebaseDatabase.getInstance().getReference("Products").child("Dryfruits");
        productlist_dryfruits = new ArrayList<>();
    ...
    public void mainMethod() {
        ...
        dbref_products.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                productlist_dryfruits.clear();

                for (DataSnapshot getorder : dataSnapshot.getChildren()) {
                    Product_model pm = getorder.getValue(Product_model.class);
                    productlist_dryfruits.add(pm);
                    progressDialog.dismiss();
                }

                // when list is empty text will appear
                if (!(productlist_dryfruits.size() > 0)) {
                    textView_noData_dryfruits.setVisibility(View.VISIBLE);
                    progressDialog.dismiss();
                } else {
                    textView_noData_dryfruits.setVisibility(View.INVISIBLE);
                    progressDialog.dismiss();
                }

                productAdapter = new ProductAdapter(productlist_dryfruits, getActivity());
                Collections.sort(productlist_dryfruits);
                rv_dryfruits.setAdapter(productAdapter);
