...
public class Rice extends Fragment {
    ...
    RecyclerView rv_rice;
    TextView textView_noData_rice;
    List<Product_model> productlist_rice;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rv_rice = view.findViewById(R.id.rv_dryfruits);
        textView_noData_rice = view.findViewById(R.id.tv_nodata_dryfruits);
        progressDialog = new ProgressDialog(getActivity());

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
        rv_rice.setLayoutManager(linearLayoutManager);

        dbref_products = FirebaseDatabase.getInstance().getReference("Products").child("Rice");
        productlist_rice = new ArrayList<>();
    ...
    public void mainMethod() {
        ...
        dbref_products.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                productlist_rice.clear();

                for (DataSnapshot getorder : dataSnapshot.getChildren()) {
                    Product_model pm = getorder.getValue(Product_model.class);
                    productlist_rice.add(pm);
                    progressDialog.dismiss();
                }

                // when list is empty text will appear
                if (!(productlist_rice.size() > 0)) {
                    textView_noData_rice.setVisibility(View.VISIBLE);
                    progressDialog.dismiss();
                } else {
                    textView_noData_rice.setVisibility(View.INVISIBLE);
                    progressDialog.dismiss();
                }

                productAdapter = new ProductAdapter(productlist_rice, getActivity());
                Collections.sort(productlist_rice);
                rv_rice.setAdapter(productAdapter);
